...
def render_simple_chart(df: pd.DataFrame):
    ...
    try:
        ...
        if not PLOTLY_AVAILABLE: # Added this check
       ...
       ENV_DEFAULTS["AZURE_OPENAI_CHAT_DEPLOYMENT_NAME"] and ENV_DEFAULTS["AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME"]:
        try:
            app_config = Config(
                ...
                # Ensure your Config class definition includes default values for all other parameters
                # or that they are loaded from os.getenv() within the Config class itself.
            )
...
with st.sidebar:
    ...
    if rag:
        # Azure OpenAI is the only supported service now
...
if apply_button:
    # Build the config parameters dictionary for Config class instantiation
    # This should include all parameters Config expects, using form values or falling back to existing/defaults.
    
    # Start with a base configuration from existing rag system or a new Config instance for defaults
    base_config_params = {}
    if rag and hasattr(rag, 'config'):
        base_config_params = rag.config.__dict__.copy()
    else:
        # Attempt to get defaults from a new Config instance if rag.config is not available
        # This assumes Config() can be called without arguments and loads its own defaults (e.g., from .env)
        try:
            base_config_params = Config().__dict__.copy()
        except Exception: # If Config() requires arguments, this might fail. Fallback to an empty dict or hardcoded defaults.
             pass # Or handle more gracefully by defining all necessary defaults here
    ...
    updated_params_from_form = {
        ...
        # Ensure openai_api_key is not passed or is None, as fallback is removed
        "openai_api_key": None,
    }
    
    # Merge base_config_params with updated_params_from_form, giving priority to form values
    final_config_params = {**base_config_params, **updated_params_from_form}

    # Remove openai_api_key explicitly if it was in base_config_params and should be None now
...
# (No changes from previous version of this tab needed based on the request)
with tab_analytics: 
    if rag:
        ...
        with col4:
            query_count_user = len([m for m in st.session_state.messages if m['role'] == 'user']) 
        ...
        st.caption("過去30日間の質問数の推移を表示しています（シミュレーションデータ）") 
        ...
        if PLOTLY_AVAILABLE:
            ...
            if not query_data.empty:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    marker=dict(size=8, color='var(--accent-hover)'), 
                    fill='tozeroy',
                ...
                fig.update_layout(
                    ...
                    xaxis=dict(showgrid=False, title="日付"), 
                    yaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.1)', title="質問数"), 
                    hovermode='x unified'
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("質問履歴データがありません。") 

        else:
            st.warning("Plotlyが利用できません。グラフは表示されません。`pip install plotly` を実行してください。") 


        st.markdown("### 🕐 最近の質問 (ユーザー)") 
        user_questions_list = [msg for msg in st.session_state.messages if msg["role"] == "user"]
        if user_questions_list:
            recent_questions_data = []
            for i, msg in enumerate(reversed(user_questions_list[-10:])): 
                content_preview = msg["content"][:100] + "..." if len(msg["content"]) > 100 else msg["content"]
                recent_questions_data.append({"質問": content_preview}) 
            if recent_questions_data:
                st.dataframe(pd.DataFrame(recent_questions_data), use_container_width=True, hide_index=True)
            else: 
                st.info("まだユーザーからの質問がありません。") 
        else:
            st.info("まだユーザーからの質問がありません。") 
    else:
...
with tab_data: 
    if not rag:
        ...
        st.warning("RAGシステムがデータテーブル管理機能（create_table_from_file, get_data_tables, delete_data_tableのいずれか）をサポートしていません。rag_system_enhanced.py を確認してください。") 
    else:
...
with tab_settings: 
    ...
    if apply_settings_button_form:
        try:
            ...
            # It's important that the Config class can handle all these parameters
            # and has appropriate defaults or ways to load them (e.g., from os.getenv)
            # for any parameters not explicitly provided by the form.
            new_app_config_obj = Config(**form_values)

            cfg_changed_flag = False 
            if rag and hasattr(rag, 'config'):
                # Compare all fields of the new config with the old one
                for field_name in new_app_config_obj.__dict__:
                    # Use getattr to safely access attributes, provide a default if attribute might not exist in old config
    ...
    if reset_settings_button_form:
        ...
        # Create a Config instance that reflects the .env defaults for all fields.
        # This relies on Config class correctly loading .env or having hardcoded defaults.
        # Then, ensure the specific ENV_DEFAULTS for Azure are applied, and OpenAI key is None.
        default_config_for_reset_obj = Config() # Load initial defaults
        
        # Override with ENV_DEFAULTS values that are critical for reset state
        ...
        default_config_for_reset_obj.openai_api_key = None # Ensure no OpenAI fallback
        ...
        # Other fields will take their defaults from Config() or os.getenv() as defined in Config
    ...
    if rag and hasattr(rag, 'config'):
        ...
        for key in sensitive_keys:
            ...
            elif key == "openai_api_key" and key in config_display_dict : # Ensure None is displayed clearly
